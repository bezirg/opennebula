{-# LANGUAGE LambdaCase #-}
module Template (toPlain) where

import Text.XML.Light 
import qualified Text.XML.Light.Cursor as Z
import Data.List (intersperse)

toPlain :: String -> Maybe Template
toPlain s = do
  doc <- parseXMLDoc s
  templElem <- findElement (QName "TEMPLATE" Nothing Nothing) doc
  cleanedTempl <- cleanupTemplate templElem
  changedTempl <- changeCPUMemory 2 16000 cleanedTempl
  finalTempl <- addSlaveContext "3.3.3.3" changedTempl
  return (pTemplate finalTempl)
